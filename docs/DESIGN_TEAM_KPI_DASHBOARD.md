...
Juga dicoba memakai SCSS asli (nesting, dll) via `dartsass-rails` (mengikuti pola `zammad.scss`/`print.scss`/`knowledge_base.scss` yang sudah ada), tapi tetap gagal dengan error `sassc` LoadError meski konfigurasi sudah mengikuti pola yang identik — penyebab pastinya belum ditemukan (kemungkinan urutan eksekusi task `dartsass:build` vs Sprockets). Diputuskan untuk **tetap pakai CSS biasa** (`app/assets/stylesheets/custom/team_kpi.css`) mengingat risiko vs manfaatnya (SCSS cuma soal kenyamanan nesting, tidak ada perbedaan hasil visual).
