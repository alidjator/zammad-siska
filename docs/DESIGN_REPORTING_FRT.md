...
**Time Accounting Types sengaja DILEWATI -- diriset lebih dalam atas permintaan user ("coba lakukan untuk time accounting type, lakukan riset dulu"), keputusan akhir tetap dilewati:**

Ditelusuri sampai ke level klik pager sungguhan (bukan cuma baca struktur controller) sebelum memutuskan:

1. `TimeAccountingTypes` adalah TAB di dalam halaman `#manage/time_accounting` (`App.ControllerTabs`), dibangun ulang tiap kali tab diklik lewat event `show.bs.tab` (`willShow` handler) -- bukan halaman dengan route sendiri. Route generik `manage/:target/:page/:search_query` yang dipakai 3 halaman di atas cuma cocok ke halaman TINGKAT ATAS (`TimeAccounting` sendiri, `ControllerTabs`), yang `show()` bawaannya (diwariskan dari `ControllerTabs`) TIDAK menerima/meneruskan param apa pun ke tab di dalamnya.
2. Ditelusuri langsung `App.ControllerTable#paginate` (klik pager native, `table.coffee` sekitar baris 795) -- kalau `pagerAjax` aktif, klik nomor halaman SELALU memanggil `@navigate("#{pagerBaseUrl}#{halaman}/...")`, hardcode, tidak bisa diganti callback lain. Karena tidak ada route yang benar-benar terhubung balik ke state tab ini, klik "halaman 2" hasilnya HANYA mengubah URL browser tanpa mengambil data halaman 2 yang benar -- atau memicu seluruh halaman `TimeAccounting` dimuat ulang dan tab kembali ke halaman 1. Ini kegagalan FUNGSIONAL, bukan sekadar beda gaya kode.
3. Ditelusuri lebih dalam lagi `App.ControllerGenericIndex#render` -- mekanisme "ambil data pakai `page`/`per_page` dari server" dan "pager native yang bergantung pada `@navigate`" TERNYATA dikontrol SATU flag yang sama (`pageData.pagerAjax`), tidak bisa diaktifkan terpisah.

**Kesimpulan riset**: mengonversi genuinely butuh salah satu dari dua hal yang sama-sama tidak sepadan manfaatnya: (a) membuang `ControllerGenericIndex` untuk tab ini dan menulis ulang SELURUH mekanisme CRUD (fetch, render, create/edit/delete, import) secara manual, bukan sekadar menambah pagination; atau (b) mengubah kode INTI `ControllerTable`/`ControllerGenericIndex` yang dipakai BERSAMA puluhan halaman Admin lain, menyebarkan risiko ke seluruh aplikasi untuk manfaat 1 tab kecil. Ditambah data Activity Types biasanya cuma segelintir baris. Disampaikan ke user sebelum menulis kode apa pun -- user setuju: **"lewati"**. Keputusan final, bukan sekadar dilewati sementara.
...
Ini SECARA TEKNIS bisa dikonversi ke server-side genuinely paginated (kirim `page`/`per_page` di tiap poll, server `LIMIT`/`OFFSET`) -- TAPI berbeda kategori risiko dari semua tabel Admin di atas: Overviews dipakai oleh SELURUH agent, TERUS-MENERUS (bukan sesekali oleh admin), sudah dites bertahun-tahun dalam bentuk sekarang, dan mekanismenya (poll 2,8 detik) berinteraksi dengan hal lain yang tidak disentuh sesi ini (indikator "ada tiket baru", urutan real-time, dst.) -- perubahan yang salah langkah di sini bisa berdampak ke SEMUA agent secara bersamaan, jauh berbeda dari 3 tabel Admin (dipakai jarang, oleh admin saja) yang baru dikonversi.

**KEPUTUSAN FINAL: TIDAK dikonversi.** Sebelum diputuskan, ditelusuri dulu bagaimana Zammad sendiri menangani ini di implementasi yang LEBIH BARU (`app/graphql/gql/queries/tickets/cached/by_overview.rb`, dipakai UI Vue/mobile Zammad generasi baru yang TIDAK dipakai di deployment ini tapi tetap ada di source code) -- ditemukan komentar eksplisit dari tim inti Zammad sendiri: *"Frontend needs to fetch all visible tickets at once, that can be up to 1000."* Bahkan versi terbaru Zammad pun SENGAJA tidak membangun Overviews dengan `LIMIT`/`OFFSET` per-halaman seperti tabel Admin -- tetap pola "ambil semua sampai batas tertentu sekaligus". Ini bukan cuma soal risiko implementasi, tapi bertentangan secara arsitektural: OFFSET-based pagination di atas data yang urutannya SERING berubah antar-poll (tiket diurutkan `updated_at`/prioritas, kolom yang terus berganti) berisiko membuat tiket duplikat di 2 halaman atau hilang dari keduanya sekaligus -- bug klasik yang sulit terlihat saat QA tapi nyata mengganggu agent sehari-hari. Temuan ini disampaikan ke user sebelum menulis kode apa pun, dan user memutuskan **Overviews tidak perlu diubah** -- katup pengaman yang memang disediakan Zammad untuk kekhawatiran performa adalah Setting `ui_ticket_overview_ticket_limit` yang sudah diberi UI (lihat `docs/TASKLIST_SISKA.md` "Di Luar Fase — Setting Batas Tiket per Overview"), bukan pagination jaringan per-halaman.
...
### Setting Bersama untuk Ukuran Halaman Semua Tabel Admin > Manage

User bertanya "apakah ada tabel yang limit perpage nya masih belum configurable?" -- disurvei lewat `grep` ke seluruh app (`pagerPerPage` di semua controller): ditemukan **17 halaman** (14 native Zammad -- Groups, Roles, Users, Organizations, Triggers, Macros, Templates, Webhooks, Jobs, Text Modules, Report Profiles, Core Workflow, Public Links, Overview config -- PLUS 3 yang baru dikonversi Section 8 di atas -- Ticket States, Ticket Priorities, Checklist Templates) semuanya hardcode `pagerPerPage: 50` langsung di CoffeeScript, tidak satu pun bisa diubah lewat Setting. Ini konvensi native Zammad sendiri, bukan sesuatu yang terlewat khusus di proyek ini -- disampaikan apa adanya ke user sebelum bertindak.

Atas permintaan lanjutan "tambahkan untuk semuanya", dibuat **SATU Setting bersama** (`ui_admin_list_per_page`, area `SISKA::AdminListPagination`, default 50, `frontend: true`, `script/create_admin_list_per_page_setting.rb`) -- bukan 17 Setting terpisah, karena ke-17 halaman ini secara konseptual sama persis (ukuran halaman daftar CRUD Admin generik yang dibangun di atas `App.ControllerGenericIndex`/`App.ControllerTable` yang sama), beda dari `report_preview_per_page`/`aux_status_manage_per_page` yang memang mewakili domain/fitur custom yang berbeda satu sama lain.

Ke-17 file diubah dari `pagerPerPage: 50` (angka tetap) jadi `pagerPerPage: parseInt(App.Config.get('ui_admin_list_per_page'), 10) || 50` -- default TETAP 50 kalau Setting kosong/belum diisi, jadi perilaku tidak berubah sampai admin benar-benar mengubah nilainya lewat Admin > Settings > SISKA > **Admin List** (tab baru di `siska_settings.coffee`). Setting ini MURNI dibaca frontend -- backend `IndexFull`/`SearchFull` native sudah menerima `per_page` apa pun yang dikirim client tanpa whitelist, jadi tidak ada perubahan backend sama sekali untuk ini.

**Catatan cakupan penting**: 14 dari 17 file yang diubah adalah kode ASLI Zammad (bukan buatan proyek ini) -- ini perubahan dengan blast radius lebih luas dari yang biasa dilakukan proyek ini (biasanya cuma menyentuh halaman/fitur custom sendiri). Perubahannya sendiri sangat mekanis dan rendah risiko (mengganti literal angka tetap dengan pembacaan Setting yang defaultnya identik), tapi tetap dicatat secara eksplisit di sini karena menyentuh banyak file inti sekaligus.

Diverifikasi: `Setting.get('ui_admin_list_per_page')` mengembalikan 50 pasca dibuat, diuji ubah ke 30 lalu kembali ke 50 tanpa error, `frontend: true` dikonfirmasi (akan terkirim ke client lewat `SessionsController#config_frontend`). Compiled bundle dikonfirmasi mengandung `ui_admin_list_per_page` di 17 titik (sesuai jumlah file yang diubah). Container boot bersih pasca deploy. **Verifikasi interaktif (ubah Setting dari Admin UI, lihat ukuran halaman berubah di salah satu dari 17 tabel) belum dilakukan user di sesi ini.**
