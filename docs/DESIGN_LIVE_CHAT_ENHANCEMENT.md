...
**Lingkup fitur ini SENGAJA cuma sisi AGENT**: sempat ditanya user "kalau dari sisi customer?" setelah fitur ini diverifikasi -- dikonfirmasi ulang bahwa keputusan awal (klarifikasi AskUserQuestion di awal desain fitur ini) memang "riwayat chat SEBELUMNYA dari customer yang sama" dilihat OLEH AGENT, bukan customer melihat riwayat dirinya sendiri. Widget customer (`public/assets/chat/`) TIDAK punya dan TIDAK dirancang fitur ini sama sekali -- kalaupun mau dibangun, perlu mekanisme identifikasi visitor terpisah (mis. `localStorage`) karena widget anonim, ini akan jadi fitur baru terpisah kalau diminta.

**Perluasan (setelah verifikasi pertama): transkrip LENGKAP, bukan cuma tanggal+link tiket**. User mencoba fitur ini secara langsung dan minta "previous chat itu terdapat isi chat komplit sebelumnya", bukan cuma tanggal + link "Open Ticket" (yang mengharuskan pindah ke tiket lama untuk baca ulang). Diperluas:
- **Backend** (`chat_session_start.rb#previous_session_summary`): tiap entri riwayat sekarang menyertakan array `messages` (isi lengkap `session.messages`, urut waktu), masing-masing dengan `content`, `is_from_agent` (ditentukan PERSIS seperti `sync_message_to_ticket` -- `created_by_id == session.user_id`, bukan logika baru), dan `created_at`.
- **UI agent** (`customer_chat/chat_window.jst.eco`): tiap entri riwayat dibungkus `<details>`/`<summary>` NATIF HTML (collapsed by default, TANPA JS tambahan) supaya panel tidak langsung penuh kalau ada beberapa sesi sebelumnya (dibatasi 5 sesi, tapi transkrip penuh 5x bisa panjang) -- klik ringkasan (tanggal + link tiket) untuk membuka/tutup transkripnya. Style baru di `chat_enhancements.scss` (`.chat-previous-session`) -- indentasi + warna pudar, konsisten pola visual `.chat-message-quote` yang sudah ada.
- Diverifikasi lewat Puppeteer: memanggil `App.view('customer_chat/chat_window')` langsung dengan data riwayat contoh (bypass alur accept-chat penuh yang lambat/rapuh di lingkungan pengujian ini), dikonfirmasi transkrip lengkap dengan label "Customer:"/"Agent:" muncul benar di dalam `<details>` per sesi.

**Bug KEDUA ditemukan setelah user melaporkan "masih tidak menampilkan isi chat sebelumnya" walau sudah hard-refresh browser**: root cause SEBENARNYA bukan cache browser (dicek: baik halaman utama maupun bundle JS sama-sama `Cache-Control: no-store`, jadi tidak mungkin basi) -- melainkan `previous_sessions` (dan transkripnya) ternyata HANYA dihitung & dikirim SEKALI, di dalam `Sessions::Event::ChatSessionStart#run` (saat agent accept chat BARU). Jendela chat yang RECONNECT -- dipicu event `chat_status_agent` (heartbeat berkala + saat agent reload halaman untuk chat yang MASIH BERJALAN) yang memanggil `Chat::Session.self.active_chats_by_user_id` (method NATIVE Zammad, bukan buatan Fase 5) -- membangun ulang data sesi TANPA field `previous_sessions` sama sekali. Direproduksi & dikonfirmasi lewat Puppeteer (`verify_reload_repro.js`): sebelum reload, panel riwayat + transkrip muncul benar (5 `<details>`); setelah `agent.reload()`, jumlah `<details>` jatuh ke 0 -- seluruh seksi "Previous chats from this visitor" LENYAP TOTAL, bukan cuma transkripnya.

**Diperbaiki dengan memusatkan logikanya** -- dipindah dari method privat di `chat_session_start.rb` jadi method instance publik `Chat::Session#previous_sessions_summary` (dan helper privat `previous_session_summary_entry`), lalu dipanggil dari KEDUA jalur: `ChatSessionStart#run` (accept baru) DAN `Chat::Session.active_chats_by_user_id` (reconnect/reload). Setelah fix, direproduksi ulang dengan skenario SAMA PERSIS (`verify_reload_repro.js`) -- hasil SEBELUM dan SESUDAH reload identik (5 `<details>`, isi sama), mengonfirmasi riwayat bertahan lewat reload halaman, bukan cuma tampil sesaat setelah accept pertama kali.
...
#### 5.3.1 Temuan Pasca-Implementasi dari Pengujian Browser Sungguhan (Puppeteer)

Section 5.3 di atas sudah lolos verifikasi `rails runner`/`curl` saat awal ditulis, TAPI verifikasi itu tidak cukup -- dua bug nyata baru ketahuan setelah dites lewat browser sungguhan (klik + screenshot asli, bukan simulasi event langsung di proses `rails runner`), dan satu bug CSS baru ketahuan dari laporan user sendiri (screenshot beranotasi tangan):

1. **Payload `reply_to` tidak sampai ke penerima balasan.** Poin 4 di atas ("Broadcast ... kode yang sudah ada di `run` cukup diteruskan apa adanya karena `chat_message` di-serialize utuh") ternyata SALAH ASUMSI -- `chat_message` sebagai object ActiveRecord mentah, saat diserialize ke JSON, HANYA menyertakan kolom (`reply_to_id` sebagai angka polos), bukan isi asosiasi `reply_to` yang dirujuknya. Akibatnya hanya PENGIRIM balasan yang melihat kutipan (dari state lokal optimistic di browser-nya sendiri), penerima tidak pernah melihatnya. Diperbaiki di `lib/sessions/event/chat_session_message.rb`: bangun `message_payload` eksplisit (`chat_message.attributes` + `reply_to: {content: ...}` kalau ada) sebelum broadcast ke KEDUA sisi.
2. **Fix di atas sudah benar & lolos `rails runner`, tapi TETAP tidak muncul di browser sungguhan.** Root cause: proses live chat SISKA berjalan di 2 CONTAINER TERPISAH -- `zammad-app` (Rails/HTTP, tempat semua deploy fix Fase 5 sejauh ini di-`docker cp`-kan) dan `zammad-websocket` (proses KHUSUS menjalankan `script/websocket-server.rb`, yang SESUNGGUHNYA memproses event chat live) -- keduanya TIDAK berbagi filesystem/volume. `rails runner` tidak pernah mendeteksi celah ini karena jalur pengujian itu (`Sessions::Event.run`) berjalan langsung di proses `rails runner` sendiri, sama sekali tidak melalui `zammad-websocket`. **Disiplin baru ke depan**: setiap perubahan pada `lib/sessions/event/chat_*.rb` atau model terkait (`Chat::Session`, `Chat::Message`) WAJIB di-`docker cp` ke KEDUA container (`zammad-app` dan `zammad-websocket`), tidak cukup salah satu saja.
3. **Ikon reply di WIDGET CUSTOMER nyaris tidak terlihat + memicu scrollbar horizontal.** Dilaporkan user (screenshot beranotasi tangan) menyangka fitur reply "tidak ada" di sisi customer -- ternyata kodenya sudah ada & ter-build benar, masalahnya CSS. Di `public/assets/chat/chat.scss`, rule `.zammad-chat-message-reply` ditulis dengan `opacity: 0` (baru terlihat saat `:hover`, konvensi yang wajar untuk AGENT sebagai pengguna tool profesional harian, tapi tidak untuk CUSTOMER anonim yang tidak terbiasa meng-hover mencari fitur tersembunyi) dan `right/left: -1.6em` (posisi DI LUAR kotak bubble lewat margin negatif, memperlebar konten pesan melebihi lebar container -> scrollbar horizontal). CSS setara di sisi agent (`app/assets/stylesheets/chat_enhancements.scss`, `.chat-message-reply`) ternyata SUDAH benar sejak awal (`top:4px; right:4px`, di dalam bubble) -- mengonfirmasi ini murni bug di stylesheet widget yang ditulis terpisah. Diperbaiki (percobaan pertama): posisi dipindah ke dalam bubble, opacity dinaikkan jadi selalu terlihat sebagian (`0.5`, penuh `1` saat hover).
4. **Susulan dari poin 3 -- ikon TERNYATA masih salah tempat di kondisi nyata** (dilaporkan lagi oleh user, masih dari sesi pengujian mandiri): ikon mengambang jauh di kanan (dekat scrollbar), bukan menempel di bubble. Root cause SEBENARNYA: `position: relative` (fix poin 3) dipasang di `.zammad-chat-message` (elemen WRAPPER LUAR, lebar penuh/`display:block`), padahal bubble yang benar-benar terlihat adalah `.zammad-chat-message-body` (`inline-block`, `max-width: 70%`) -- ikon reply adalah SIBLING dari `.zammad-chat-message-body` di markup (`views/message.eco`), bukan anaknya, jadi posisi absolutnya mengikuti wrapper lebar-penuh, bukan bubble sempit. Struktur ini beda dari sisi agent, di mana `.chat-message` (yang dipasangi `position: relative`) MEMANG elemen bubble itu sendiri, bukan wrapper terpisah -- sehingga solusi poin 3 (menyalin NILAI CSS agent) tidak menyalin STRUKTUR-nya, dan cukup untuk uji injeksi markup saat itu tapi tidak untuk kondisi nyata. Diperbaiki dengan memindahkan markup ikon jadi ANAK `.zammad-chat-message-body` (bukan sibling) dan `position: relative` ikut dipindah ke situ -- sekarang meniru STRUKTUR agent, bukan cuma nilainya.

Keempatnya diverifikasi ulang lewat metode yang sama yang menemukannya (screenshot Puppeteer, untuk poin 4 ditambah pengukuran terprogram `getBoundingClientRect()` supaya tidak salah baca lagi dari sekadar tampilan visual), bukan cuma dianggap selesai dari pembacaan kode -- lihat entry 95-96, 98 `docs/ACTIVITY_LOG_SISKA.md` untuk detail pengujian.
